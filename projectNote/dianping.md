...
思路：
- 先说旁路缓存
- 再引出延迟双删（解决的问题：请求A先更新数据库，再删缓存，在这中间请求B读取旧值并将其写入缓存，导致缓存变旧值）
- 再引出解决删除缓存异常的两个方案
  - 方案1：消息队列，即删除缓存不直接删，而是将删除消息发送到消息队列，通过重试机制保证能删除，如果多次重试无效则向业务层报错
  - 方案2：Canal + 消息队列，即监控mysql的binlog日志，再写一个简单的消费者程序来异步处理binlog中的操作
    - Canal机制：利用mysql主从复制的特性，将自己伪装成mysql从节点，让主节点dump线程发送binlog日志过来，并解析
  - 方案对比：方案1会有大量业务入侵，而方案2所有变更由Canal监听维护，自动解析变更事件并发送到mq，不需要业务系统写任何发送消息的代码
- 再引出我自己项目中使用到的方案
  - 方案：redis预扣库存 + mq同步数据库库存
  - 和上面旁路缓存 + mq的区别：
    - 我的方案关注点是高并发，思想是缓存驱动数据库，操作入口是缓存，mq的作用是异步落库
    - 而旁路缓存 + mq方案关注点是写操作后的缓存一致性维护，操作入口是数据库，mq的作用是异步补偿缓存删除
  - 我的方案所需注意点：
    - 要处理失败重试、幂等、补偿机制
